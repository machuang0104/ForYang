 ...
 * Time: 下午2:49
 */
public class MRequest extends HttpRequestBase implements HttpEntityEnclosingRequest {
	...
	public MRequest(HttpMethod method) {
        super();
        this.method = method;
    }

    public MRequest(HttpMethod method, String uri) {
        super();
        this.method = method;
        setURI(uri);
    }

    public MRequest(HttpMethod method, URI uri) {
        super();
        this.method = method;
        setURI(uri);
    }

    public MRequest addQueryStringParameter(String name, String value) {
        uriBuilder.addParameter(name, value);
        return this;
    }

    public MRequest addQueryStringParameter(NameValuePair nameValuePair) {
        uriBuilder.addParameter(nameValuePair.getName(), nameValuePair.getValue());
        return this;
    }

    public MRequest addQueryStringParams(List<NameValuePair> nameValuePairs) {
    ...
    public void setRequestParams(MParams param) {
        if (param != null) {
            if (uriCharset == null) {
                uriCharset = Charset.forName(param.getCharset());
            }
            List<MParams.HeaderItem> headerItems = param.getHeaders();
            if (headerItems != null) {
                for (MParams.HeaderItem headerItem : headerItems) {
    ...
    public void setRequestParams(MParams param, RequestCallBackHandler callBackHandler) {
        if (param != null) {
            if (uriCharset == null) {
                uriCharset = Charset.forName(param.getCharset());
            }
...
//            }
    ...
    public Object clone() throws CloneNotSupportedException {
    	MRequest clone = (MRequest) super.clone();
		if (this.entity != null) {
