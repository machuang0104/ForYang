...
import com.ma.text.http.global.Error;
import com.ma.text.http.task.TaskDeal;
...
public class MHandler<T> extends PriorityAsyncTask<Object, Object, Void>
		...
		while (true) {
			...
			try {
				...
				if (MHttpUtil.sHttpCache.isEnabled(requestMethod)) {
					...
					if (result != null) {
						return new ResponseInfo<T>(null, result, true);
					}
				}
	...
	protected Void doInBackground(Object... params) {
		...
		try {
			...
			if (responseInfo != null) {
				LogUtil.i(MHttpUtil.TAG, "onSucess : " + responseInfo.responseString);
				result = TaskDeal.getTaskResult(request.getTaskId(), responseInfo);
				this.publishProgress(UPDATE_SUCCESS, result);
				return null;
			}
		} catch (HttpException e) {
			LogUtil.i(MHttpUtil.TAG, "onFaliure : " + e.getMessage());
	...
	protected void onProgressUpdate(Object... values) {
		...
		case UPDATE_FAILURE :
			...
			App.finishRequest(request.getTaskId());

			try {
				HttpException error = (HttpException) values[1];
				callback.onFailure(error, Error.getExcMsg(error.getExceptionCode()));
			} catch (Exception e) {
				e.printStackTrace();
				callback.onFailure(new HttpException(1111), Error.getExcMsg(1111));
		...
		if (statusCode < 300) {
			...
			if (entity != null) {
				...
				if (isDownloadingFile) {
					...
					result = downloadHandler.handleEntity(entity, this, fileSavePath,
							autoResume, responseFileName);
				} else {
					StringDownloadHandler downloadHandler = new StringDownloadHandler();
					result = downloadHandler.handleEntity(entity, this, charset);
					if (MHttpUtil.sHttpCache.isEnabled(requestMethod)) {
						MHttpUtil.sHttpCache.put(requestUrl, (String) result, expiry);
					}
				}
			}
			return new ResponseInfo<T>(response, (String) result, false);
