...
public class SwipeBackLayout extends RelativeLayout {
	...
	private static final int[] EDGE_FLAGS = {EDGE_LEFT, EDGE_RIGHT, EDGE_BOTTOM,
			EDGE_ALL};
	...
	private void drawShadow(Canvas canvas, View child) {
		...
		if ((mEdgeFlag & EDGE_RIGHT) != 0) {
			mShadowRight.setBounds(childRect.right, childRect.top, childRect.right
		...
		public void onViewReleased(View releasedChild, float xvel, float yvel) {
			...
			if ((mTrackingEdge & EDGE_LEFT) != 0) {
				left = xvel > 0 || xvel == 0 && mScrollPercent > mScrollThreshold
						? childWidth + mShadowLeft.getIntrinsicWidth()
								+ OVERSCROLL_DISTANCE
						: 0;
			} else if ((mTrackingEdge & EDGE_RIGHT) != 0) {
				left = xvel < 0 || xvel == 0 && mScrollPercent > mScrollThreshold
