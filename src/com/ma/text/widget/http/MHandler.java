...
import com.ma.text.client.http.action.ActionDeal;
import com.ma.text.client.http.global.ErrorMy;
import com.ma.text.tools.ResUtil;
import com.ma.text.widget.log.LogUtil;

@Deprecated
public class MHandler<T> extends PriorityAsyncTask<Object, Object, Void>
	...
	public MHandler(AbstractHttpClient client, HttpContext context,
			String charset, MCallBack<T> callback) {
		this.client = client;
	...
	public void setRequestCallBack(MCallBack<T> callback) {
		this.callback = callback;
	}

	public MCallBack<T> getRequestCallBack() {
	 ...
	 * sendRequest: 执行请求
	 */
	private MResponseInfo<T> sendRequest(HttpRequestBase request)
		...
		while (true) {
			...
			try {
				...
				if (MHttpUtil.sHttpCache.isEnabled(requestMethod)) {
					String result = MHttpUtil.sHttpCache.get(requestUrl);
					if (result != null) {
						return new MResponseInfo<T>(null, result, true);
					}
				}

				MResponseInfo<T> responseInfo = null;
				if (!isCancelled()) {
	...
	protected Void doInBackground(Object... params) {
		...
		try {
			...
			MResponseInfo<T> responseInfo = sendRequest(request);
			Object result;
			if (responseInfo != null) {
				LogUtil.d(MHttpUtil.TAG, "onSucess : "
						+ responseInfo.responseString);
				result = ActionDeal.getTaskResult(request.getActionId(),
						responseInfo);
				this.publishProgress(UPDATE_SUCCESS, result);
				return null;
			}
		} catch (HttpException e) {
			LogUtil.d(MHttpUtil.TAG, "onFaliure : " + e.getMessage());
			e.printStackTrace();
	...
	protected void onProgressUpdate(Object... values) {
		...
		case UPDATE_FAILURE:
			...
			try {
				HttpException error = (HttpException) values[1];
		...
		case UPDATE_SUCCESS:
			...
			callback.onSuccess((Response<T>) values[1]);
			break;
		default:
			break;
		}
	...
	private MResponseInfo<T> handleResponse(HttpResponse response)
		...
		if (statusCode < 300) {
			...
			if (entity != null) {
				...
				if (isDownloadingFile) {
					...
					if (MHttpUtil.sHttpCache.isEnabled(requestMethod)) {
						MHttpUtil.sHttpCache.put(requestUrl, (String) result,
								expiry);
					}
				}
			}
			return new MResponseInfo<T>(response, (String) result, false);
	...
	public void cancel() {
		...
		if (callback != null) {
			...
			callback.onFailure(new HttpException(ErrorMy.Code.FAIL_NET_DEAD),
					ResUtil.getStr(R.string.tip_network_weak));
		}
	}
